			...
			rl.question('Type an option: ', (option) => {
				switch (option) {
					...
					case "11":
						...
						db.collection('companies').find({ "name": "Facebook", "relationships.is_past": false }, { 'relationships.person.permalink': 1, _id: 0 }).toArray((error, result) => {
							if (error) {
								console.log(error);
								rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
							} else {
					...
					case "12":
						...
						db.collection('companies').find({ "relationships.is_past": true, 'relationships.person.permalink': "david-ebersman" }, { "name": 1, _id: 0 }).toArray((error, result) => {
							if (error) {
								console.log(error);
								rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
							} else {
					...
					case "14":
						...
						db.collection('companies').find({'tag_list': { $regex: new RegExp('(social-networking)')}}, { "name": 1, _id: 0 }).toArray((error, result) => {
							if (error) {
								console.log(error);
								rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
							} else {
					...
					case "15":
						...
						db.collection('companies').find({ 'tag_list': { $regex: new RegExp('(social-networking)') }, "founded_year": { $gte: 2002, $lte: 2016 } }, { "name": 1, _id: 0 }).count((error, result) => {
							if (error) {
								console.log(error);
								rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
							} else {
					...
					case "17":
						...
						db.collection('companies').find({ 'tag_list': { $regex: new RegExp('(social-networking)') }, "founded_year": { $gte: 2002, $lte: 2016 }, 'offices.city': 'New York' }, { "name": 1, _id: 0 }).count((error, result) => {
							if (error) {
								console.log(error);
								rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
							} else {
